    ...
    <section id="process" className="py-24 relative bg-white border-y border-gray-200">
      <div className="container max-w-6xl mx-auto px-16 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-20">
          <h2 className="text-3xl md:text-5xl font-bold mb-6 text-[#0A0F1C]">How it works</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
          ...
          <div className="hidden md:block absolute top-[45px] left-[10%] right-[10%] h-[2px] bg-gradient-to-r from-[var(--color-electric-green)]/20 via-[var(--color-cobalt)]/50 to-[var(--color-electric-green)]/20 z-0" />
          ...
          {steps.map((step, idx) => (
            ...
            >
              <div className="w-24 h-24 rounded-full bg-white border-2 border-gray-200 flex items-center justify-center mb-6 shadow-xl relative">
                {step.icon}
                <div className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-[var(--color-cobalt)] text-white flex items-center justify-center text-xs font-bold border-2 border-white">
                  {step.id}
                </div>
              </div>
              <h3 className="text-xl font-bold text-[#0A0F1C] mb-3">{step.title}</h3>
              <p className="text-[var(--color-tech-gray)] text-sm leading-relaxed max-w-[200px]">
                {step.description}
              </p>
            </motion.div>
          ))}
